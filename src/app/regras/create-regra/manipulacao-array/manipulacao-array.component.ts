  ...
  metodos = [
   ...
   openModalExpressao() {
    if (this.objetoLocal[`acao`][`array`]) {
      const obj = { type: this.objetoLocal[`acao`].array.tipoitems , valor: this.objetoLocal[`acao`][`expressao`] };
      this.modals.modalExpressao(this.objetoLocal[`variaveis`], obj, this.objetoLocal[`acao`]);
      this.evaluate(this.objetoLocal[`acao`].array, this.objetoLocal[`acao`][`expressao`])
    }
  }
  openVariaveis(){
    super.openVariaveis(`array`);
    this.variavelService.variavel.pipe(first(), distinctUntilChanged(), take(1)).subscribe(
      variavel => {
